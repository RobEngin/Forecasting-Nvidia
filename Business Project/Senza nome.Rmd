...
tslm_rev <- tslm(train_y ~ trend + stock, data = train_x)
...
tslm_rev <- tslm(diff(train_y) ~  net_income , data = train_x_diff)
summary(tslm_rev) # ts_employees non è significativa
...
alpha <- 0.5 # Parametro di smoothing esponenziale
...
plot(fore, main = "Forecast vs Actual Revenue", xlab = "Time", ylab = "Revenue (Billion)", xlim = c(20,25) , ylim = c(2, 10))
lines(ts_revenue, col = "black")
...
# Calcola i residui del modello ARIMAX sui dati di test
residuals <- diff(test_y) - forecast_arimax$mean
