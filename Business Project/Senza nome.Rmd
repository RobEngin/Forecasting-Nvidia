...
```{r}
tslm_rev <- tslm(ts_revenue ~ trend+season+ts_stock+ts_spending+ts_net_income+ts_employees)
...
cor_matrix <- cor(data.frame(ts_employees, ts_net_income, ts_spending, ts_revenue, ts_stock))
print(cor_matrix)
```
...
data_combined <- data.frame(
  revenue = revenue_by_quarter_2$Revenue,
  net_income = quarterly_net_income$Value,
  employees = number_of_employees_2$Valore,
  rnd_spending = rd_spending$Valore,
  stock = nvda_stock$Price
)
...
diff_revenue <- diff(ts_revenue)
diff_net_income <- diff(ts_net_income)
diff_employees <- diff(ts_employees)
diff_spending <- diff(ts_spending)
...
data_combined_diff <- data.frame(
  ...
  diff_stock = as.numeric(diff_stock)
)
...
tslm_rev <- tslm(diff_revenue ~ trend+season+diff_spending+diff_net_income+diff_stock+diff_employees)
summary(tslm_rev) # ts_employees non è significativa
